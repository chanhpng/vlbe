...
func (p *Packer) Finalize() error {
	...
	if err := verifyHeader(p.k, encryptedHeader, p.blobs); err != nil {
		...
		return fmt.Errorf("Detected data corruption while writing pack-file header: %w\nCorrupted data is either caused by hardware issues or software bugs. Please open an issue at https://github.com/chanhpng/vlbe/issues/new/choose for further troubleshooting.", err)
	}
